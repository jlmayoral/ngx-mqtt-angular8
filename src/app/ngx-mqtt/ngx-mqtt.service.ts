...
// import { connect } from 'mqtt'; // '../vendor/mqtt.min.js'; // '../vendor/mqtt.min.js';
...
// import { MqttServiceConfig, MqttClientService } from './ngx-mqtt.module';
...
export class NgxMqttService {
  ...
  public connect(opts?: IMqttServiceOptions, client?: IMqttClient) {
    ...
    if (!client) {
      this.client = <IMqttClient>mqtt.connect(this._url, mergedOptions);
    } else {
      this.client = client;
    }
  ...
  private _generalObserve(filterString: string, publishFn: Function, opts: IClientSubscribeOptions): Observable<IMqttMessage> {
    ...
    if (!this.observables[filterString]) {
      ...
      this.observables[filterString] = using(
        ...
        .pipe(
          filter((msg: IMqttMessage) => NgxMqttService.filterMatchesTopic(filterString, msg.topic)),
          publishFn(),
          refCount()
